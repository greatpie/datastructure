...
public class PolynList extends LinkList {
	...
	public PolynList(int n) throws Exception {
		getHead().setData(new PolynNode(0, -1)); // 初始化头结点中的数据元素
	...
	public int cmp(PolynNode a, PolynNode b) {
		if (a.getExpn() < b.getExpn())// a、b的指数相比较
			return -1;
	...
	public PolynList addPolyn(PolynList LA, PolynList LB) {
		Node ha = LA.getHead(); // ha指向新形成链的尾结点
		Node qa = LA.getHead().getNext(); // qa指向LA中需要计算的当前项
		Node qb = LB.getHead().getNext();// qb指向LB中需要计算的当前项
		while (qa != null && qb != null) { // qa、qb同时非空
			PolynNode a = (PolynNode) qa.getData();
			PolynNode b = (PolynNode) qb.getData();
			switch (cmp(a, b)) {
			case -1: // 多项式LA中当前结点的指数值小
				...
				break;
			case 0: // 两者的指数值相等
				double sum = a.getCoef() + b.getCoef(); // 求系数的和
				if (sum != 0.0) { // 修改多项式LA中当前结点的系数值
					a.setCoef(sum);
					ha.setNext(qa);
					ha = qa;
					qa = qa.getNext(); // 指向下一结点
					qb = qb.getNext();
				} else { // 删除多项式LA中的当前项
					qa = qa.getNext();// 指向下一结点
					qb = qb.getNext();
		...
		ha.setNext(qa != null ? qa : qb); // 插入剩余结点
		return LA;
	}
